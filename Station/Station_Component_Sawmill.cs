...
namespace Station
{
    public class Station_Component_Sawmill : Station_Component
    {
        ...
        {
            if (!actor.ActorData.CraftingUpdater.KnownRecipes.Contains(recipeName)) { Debug.Log($"KnownRecipes does not contain RecipeName: {recipeName}"); return; }
            ...
            if (!Station_Data.InventoryUpdater.InventoryContainsAllItems(cost)) { Debug.Log("Station does not have required items."); return; }
            if (!Station_Data.InventoryUpdater.HasSpaceForItems(yield)) { Debug.Log("Station does not have space for yield items."); return; }
